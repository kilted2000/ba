...
const createNewUser = async (req, res) => {
  const { body } = req;
  if (!body.firstName || !body.lastName || !body.email || !body.dogName || !body.breed || !body.personality) {
    return res.status(400).send({ status: "FAILED", data: { error: "Missing or empty required fields" } });
  }
  try {
    const newUser = { ...body };
    const createdUser = await userService.createNewUser(newUser);
